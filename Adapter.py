...
class DynamicFilter(nn.Module):
    ...
    def __init__(self, dim, med_channels=128, reweight_expansion_ratio=.25,
    ...
    def forward(self, x):
        ...
        return identity + x,promt

if __name__ == '__main__':
    block = DynamicFilter(dim=256, med_channels=128)
    input = torch.rand(6, 288, 512, 256)
    output,promt = block(input)
    print("Input size:", input.size())
    print("Output size:", output.size())
    print("promt:", output.size())
